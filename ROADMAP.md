...
1. **WISER QoL + Backend — get remaining work off the GUI thread.** Not just a cleanup: it needs a *documented, standardized pattern* so new code always does the right thing. See HANDOFF entry #1. Draft: [Standardize the off-GUI-thread pattern](ISSUE_DRAFTS.md#standardize-the-off-gui-thread-pattern). Known offenders: [dataset loading](ISSUE_DRAFTS.md#async-dataset-loading-and-rendering), [ROI average-spectra](ISSUE_DRAFTS.md#roi-average-spectra-off-the-gui-thread), stretch builder (see the stretch builder draft below). **Note:** "dataset loading" and "stretch builder" are the *same* problem — opening a dataset is slow because of the **render**, not the file open, and rendering is one shared code path that dataset open, stretch builder, and the band chooser all funnel into. Fix it once and all three improve. See HANDOFF #1.
...
- **Get things off the GUI thread** — the top priority; see the priorities section and HANDOFF #1. Draft: [Standardize the off-GUI-thread pattern](ISSUE_DRAFTS.md#standardize-the-off-gui-thread-pattern). The largest single win is the shared render path — see [async dataset loading and rendering](ISSUE_DRAFTS.md#async-dataset-loading-and-rendering).
...
- **Stretch builder responsiveness** — stretch changes can freeze WISER for *minutes* on huge images (Gale HiRISE). It updates the context pane, zoom pane, and main view — the meat of user interaction — so a blocking modal with progress is defensible, but the operation must be **cancellable** so users can reach background analysis tools. Draft: [Stretch builder cancellable + progress](ISSUE_DRAFTS.md#stretch-builder-cancellable--progress). Note this is the **same render path** that makes opening a dataset slow — see [async dataset loading and rendering](ISSUE_DRAFTS.md#async-dataset-loading-and-rendering); fixing one should fix both. Ultimately subsumed by LoD rendering.

### WISER backend

- Get things off the GUI thread (shared with QoL above). Known offenders: [async dataset loading and rendering](ISSUE_DRAFTS.md#async-dataset-loading-and-rendering) — really a *rendering* problem, and the same shared render path the stretch builder and band chooser use — and [ROI average-spectra](ISSUE_DRAFTS.md#roi-average-spectra-off-the-gui-thread).
