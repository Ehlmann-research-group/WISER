...
K-Means is currently the odd one out: it stores results in a parameter-keyed dict instead of a `RunHistoryManagerBase`-backed history (see [kmeans-run-record-refactor.md](../kmeans-run-record-refactor.md)). Issue [#613](https://github.com/Ehlmann-research-group/WISER/issues/613) is for transitioning K-Means to use the run record class.
...
**K-Means dependency:** this is cleanest if [issue #613](https://github.com/Ehlmann-research-group/WISER/issues/613) lands first, giving K-Means a `KMeansRunRecord` + `RunHistoryManagerBase` like the others, so one persistence path covers all four.

**Load:** rehydrate each manager's record list; wire records to restored datasets or place them in "closed runs".

## Describe alternatives you've considered

- **Only save a run if its output datasets are saved (initial idea).** Rejected: over-constrains; records are self-contained and the "closed runs" path already handles missing datasets — saving records always is simpler and more useful.
...
- Strong soft-prerequisite: [issue #613](https://github.com/Ehlmann-research-group/WISER/issues/613).
