...
class TestDataSerialization(unittest.TestCase):
    ...
    def test_raster_data_set(self):
        ...
        assert reconstructed_dataset.is_metadata_same(ds), "The reconstructed dataset has different metadata from the original dataset"
    ...
    def test_raster_data_band(self):
        ...
        assert reconstructed_band.is_metadata_same(band), "The reconstructed band has different metadata from the original band"
    ...
    def test_raster_data_dynamic_band_index(self):
        ...
        serializedForm = band.get_serialized_form()
        reconstructed_band: RasterDataDynamicBand = serializedForm.get_serializable_class().deserialize_into_class(
            serializedForm.get_serialize_value(),
            serializedForm.get_metadata())
    ...
    def test_numpy_array_spectrum(self):
        ...
        reconstructed_spectrum: NumPyArraySpectrum = serializedForm.get_serializable_class().deserialize_into_class(
            serializedForm.get_serialize_value(),
            serializedForm.get_metadata())
    ...
    def test_spectrum_at_point(self):
        ...
        serializedForm = spectrum.get_serialized_form()
        reconstructed_spectrum: SpectrumAtPoint = serializedForm.get_serializable_class().deserialize_into_class(
            serializedForm.get_serialize_value(),
            serializedForm.get_metadata())
