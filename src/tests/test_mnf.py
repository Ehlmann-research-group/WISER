...
class TestMnf(unittest.TestCase):
    ...
    def test_get_mnf_pipeline_matches_spy_mnf_on_caltech_fixture(self) -> None:
        ...
        try:
            ...
            image = np.asarray(dataset.get_image_data()).transpose(1, 2, 0)
            spy_noise = spectral.noise_from_diffs(image)
            spy_mnf_transform = spectral.mnf(image, spy_noise)
            spy_mnf = np.asarray(spy_mnf_transform.transform(image), dtype=np.float32)[:, :, :num_components]
