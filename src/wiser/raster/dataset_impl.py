...
emit_data_names = set(
    [
        ...
        "Gypsum_uncert",
        "Hematite_uncert",
        "Illite+Muscovite_uncert",
        "Kaolinite_uncert",
        "Montmorillonite_uncert",
        "Vermiculite_uncert",
    ]
)
...
class RasterDataImpl(abc.ABC):
    ...
    def read_default_display_bands(
        self,
    ) -> Optional[Union[Tuple[int], Tuple[int, int, int]]]:
        return None
...
class GDALRasterDataImpl(RasterDataImpl):
    ...
    def _validate_dataset(self):
        ...
        for band_index in range(1, self.gdal_dataset.RasterCount + 1):
            ...
            if self.data_ignore is None:
                ...
                raise ValueError(
                    "Cannot handle raster data with bands that "
    ...
    def reopen_dataset(self):
        ...
        file_path = (
            self.subdataset_name if self.subdataset_name is not None else file_paths[0]
        )  # Assuming the first file is the main dataset
    ...
    def sample_band_data(self, band_index, sample_factor: int):
        ...
        np_array: Union[np.ndarray, np.ma.masked_array] = band.ReadAsArray(
            buf_xsize=buf_xsize, buf_ysize=buf_ysize, resample_alg=gdal.GRIORA_Gauss
        )
        ...
        for band_index in range(1, self.gdal_dataset.RasterCount + 1):
            ...
            if "wavelength" in gdal_metadata and "wavelength_units" in gdal_metadata:
                ...
                try:
                    ...
                    logger.warn(
                        "Couldn't parse wavelength info for GDAL "
        ...
        if self._save_state == SaveState.IN_DISK_NOT_SAVED:
            try:
                if self.gdal_dataset is not None:
                    ...
                    print(
                        "Dataset variable is None. Either the dataset "
...
class PDRRasterDataImpl(RasterDataImpl):
    ...
    def get_num_dims(self):
        ...
        if self.ndims != 2 and self.ndims != 3:
            raise ValueError(
                f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_num_dims"
            )
    ...
    def get_width(self):
        if self.width is None:
            if self.ndims == 2:
                ...
                raise ValueError(
                    f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_width"
                )
    ...
    def get_height(self):
        if self.height is None:
            if self.ndims == 2:
                ...
                raise ValueError(
                    f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_height"
                )
    ...
    def num_bands(self):
        if self.number_bands is None:
            if self.ndims == 2:
                ...
                raise ValueError(
                    f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in num_bands"
                )
    ...
    def get_band_data(self, band_index, filter_data_ignore_value=True):
        ...
        if self.ndims == 2:
            ...
            raise ValueError(
                f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_band_data"
            )
    ...
    def get_all_bands_at(self, x, y):
        if self.ndims == 2:
            ...
            raise ValueError(
                f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_all_bands_at"
            )
    ...
    def get_multiple_band_data(self, band_list_orig: List[int]) -> np.ndarray:
        if self.ndims == 3:
            ...
            raise ValueError(
                f"The number of dimensions this raster has is {self.ndims} "
    ...
    def get_all_bands_at_rect(self, x: int, y: int, dx: int, dy: int):
        if self.ndims == 2:
            ...
            raise ValueError(
                f"PDR Raster has neither 2 or 3 dimensions. Instead has {self.ndims} in get_all_bands_at_rect"
            )
...
class ENVI_GDALRasterDataImpl(GDALRasterDataImpl):
    ...
    def save_dataset_as(
        ...
        if src_offset_x + dst_width > src_width:
            raise ValueError(
                f'Sum of "left" value {src_offset_x} and '
        ...
        if src_offset_y + dst_height > src_height:
            raise ValueError(
                f'Sum of "top" value {src_offset_y} and '
        ...
        logger.debug(
            f'driver.Create("{path}", {dst_width}, {dst_height}, '
            + f"{dst_bands}, {gdal_elem_type}, {driver_options})"
        )

        dst_gdal_dataset = driver.Create(
            path, dst_width, dst_height, dst_bands, gdal_elem_type, driver_options
        )
        ...
        if len(dst_wavelengths) == dst_bands:
            ...
            print(
                f"WARNING:  # bands with wavelengths {len(dst_wavelengths)}"
    ...
    def read_default_display_bands(self) -> Union[Tuple[int], Tuple[int, int, int]]:
        ...
        if "default_bands" in md:
            ...
            if s[0] != "{" or s[-1] != "}":
                raise ValueError(
                    f"ENVI file has unrecognized format for default bands:  {s}"
                )
