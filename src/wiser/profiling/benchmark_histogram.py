...
def test(dataset_path):
    ...
    # Create an application state, don't need an app for it, just leave it as None
    app_state = ApplicationState(app=None)

    raster_pane = RasterPane(app_state)
    app_state.add_dataset(dataset)
    # Create raster pane, give it parameter application state that contains the data sets


    # Try calling show datasets on it, but it might call create pix map
