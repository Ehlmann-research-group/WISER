...
def measure_bandmath_time(
    equation: str, variables: Dict[str, Tuple[VariableType, Any]], use_old_method=False
):
    """
    A helper function for measuring the time it takes to call eval_bandmath_expr
    """
    expr_info = get_bandmath_expr_info(equation, variables, {})
    result_name = "test_result"

    cache = DataCache()

    start_time = time.perf_counter()
    (_, result_dataset) = bandmath.eval_bandmath_expr(
        equation, expr_info, result_name, cache, variables, {}, use_old_method
    )
...
def stress_test_benchmark(
    large_band_dataset_path: str,
    normal_image_cube_path: str,
    large_image_cube_path: str,
    use_both_methods=False,
    use_old_method=False,
    N=1,
    output_file="output/bandmath_benchmark.txt",
):
    """
    ...
    normal_spectrum = normal_image_cube.get_all_bands_at(
        normal_image_cube.get_width() / 2, normal_image_cube.get_height() / 2
    )
    large_spectrum = large_image_cube.get_all_bands_at(
        large_image_cube.get_width() / 2, large_image_cube.get_height() / 2
    )

    variables = {
        "b1": (VariableType.IMAGE_BAND, b1),
    ...
    for key, value in stressing_equation_dict.items():
        ...
        for iter in range(N):
            ...
            if use_both_methods:
                ...
                print("New method calculating!")
                time_new_method, result_new_method = measure_bandmath_time(
                    value, variables, use_old_method=False
                )
                print("New method done calculating!")
                if results_new_method[key] is None:
                    results_new_method[key] = result_new_method
                print(f"time_new_method: {time_new_method}")

                print("Old method calculating!")
                time_old_method, result_old_method = measure_bandmath_time(
                    value, variables, use_old_method=True
                )
                print("Old method done calculating!")
                ...
                print("Old method calculating!")
                time, _ = measure_bandmath_time(
                    value, variables, use_old_method=use_old_method
                )
                print("Old method done calculating!")
                time_outer = time
            else:
...
def test_both_methods(hdr_paths, N=1):
    ...
    for hdr_file in hdr_files:
        ...
        variables = {
            "a": (VariableType.IMAGE_CUBE, dataset),
        ...
        for key, value in equation_dict.items():
            ...
            print(f"Equation: {value}")
            print("New method calculating")
            _, result_new_method = measure_bandmath_time(
                value, variables, use_old_method=False
            )
            print("New method done calculating")
            if results_new_method[key] is None:
                results_new_method[key] = result_new_method
            print("Old method calculating!")
            _, result_old_method = measure_bandmath_time(
                value, variables, use_old_method=True
            )
            print("Old method done calculating!")
            ...
            print(
                f"np.assertequal: {np.allclose(arr_new_method, arr_old_method, equal_nan=True)}"
            )
            if results_old_method[key] is None:
                results_old_method[key] = result_old_method
            are_close = np.allclose(
                arr_new_method, arr_old_method, rtol=1e-4, equal_nan=True
            )
...
if __name__ == "__main__":
    ...
    dataset_500mb_copy = (
        "c:\\Users\\jgarc\\OneDrive\\Documents\\Data\\caltech-pic-copy.hdr"
    )
    ...
    stress_test_benchmark(
        dataset_15gb, dataset_500mb, dataset_6GB, use_both_methods=True, N=N
    )
