...
def profile_5GB_mean_time(dataset_path: str) -> None:
    roi_path = "C:\\Users\\jgarc\\OneDrive\\Documents\\Data\\Task1.2_Slow_ROI_Mean_5gb_285_spectra\\big_roi_5b_worth.geojson"
    loader = RasterDataLoader()
    dataset = loader.load_from_file(dataset_path)
    rois = roi_export.import_geojson_file_to_rois(roi_path)

    
    profiler = cProfile.Profile()
    profiler.enable()
    print('================Enabled Profile================')
    spectrum = calc_roi_spectrum(dataset, rois[0])
    profiler.disable()
    print('================Disabled Profile================')

    # Save the profiling stats to a file
    with open(f"output/roi_fast_profile.txt", "w") as f:
        ps = pstats.Stats(profiler, stream=f)
        ps.sort_stats("tottime")
        ps.print_stats()
...
if __name__ == '__main__':
    ...
    dataset_path = "C:\\Users\\jgarc\\OneDrive\\Documents\\Data\\Task1.2_Slow_ROI_Mean_5gb_285_spectra\\RhinoLeft_2016_07_28_12_56_01_SWIRcalib_atmcorr_expanded_lines_and_samples_2.hdr"
    profile_5GB_mean_time(dataset_path)
