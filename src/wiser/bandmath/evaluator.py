...
class AsyncTransformer(lark.visitors.Transformer):
    ...
    async def _transform_tree(self, tree):
        ...
        children_tasks = [
            asyncio.create_task(self._transform_children(tree.children))
        ]  # for c in tree.children] #  [child async for child in self._transform_children(tree.children)]
...
def eval_bandmath_expr(
    ...
    *   VariableType.NUMBER:  float
    ...
    if test_parallel_io or (
        expr_info.result_type == VariableType.IMAGE_CUBE
        and should_chunk
