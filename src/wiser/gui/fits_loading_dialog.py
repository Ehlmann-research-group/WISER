...
class FitsDatasetLoadingDialog(QDialog):
    ...
    def _init_interpretation_options(self):
        ...
        if self._naxis == 2:
            ...
            raise Exception(
                "WISER does not support naxis number other than 2 or 3 for datasets. "
    ...
    def _on_interp_opt_changed(self):
        ...
        data_varying_options = self._ui.data_vary_combo
        if (
            interpretation_options.currentData() == DataType.IMAGE_CUBE
            or interpretation_options.currentData() == DataType.SINGLE_IMAGE_BAND
        ):
            data_varying_options.setEnabled(False)
        else:
...
class FitsSpectraLoadingDialog(QDialog):
    ...
    def accept(self):
        ...
        for i in range(len(y_arrays)):
            numpy_spectrum = NumPyArraySpectrum(
                arr=y_arrays[i],
                name=f"{spectrum_name}_{i}",
        ...
        self.spectral_library = ListSpectralLibrary(
            numpy_spectrum_list, name=spectrum_name, path=self._filepath
        )
