...
from .app_state import StateChange
...
class SpectrumPointDisplayInfo:
    ...
    def generate_plot(self, axes, font_name: str, font_size: int):
        ...
        self._label = axes.text(
            0.02,
            0.98,
            label,
            fontproperties=selection_font,
...
class SpectrumPlot(QWidget):
    ...
    def __init__(self, app, parent=None):
        ...
        self._app_state.active_spectrum_changed.connect(
            self._on_active_spectrum_changed
        )
        ...
        self._app_state.spectral_library_removed.connect(
    ...
    def _init_ui(self):
        ...
        self._act_collect_spectrum = add_toolbar_action(
            self._toolbar,
            ":/icons/collect-spectrum.svg",
            self.tr("Collect spectrum"),
            self,
        )
        ...
        self._act_configure = add_toolbar_action(
            self._toolbar, ":/icons/configure.svg", self.tr("Configure"), self
        )
    ...
    def _update_spectrum_mouse_click(
        ...
        self._click = SpectrumPointDisplayInfo(
            spectrum,
            index,
            self._plot_uses_wavelengths,
            band_units=self._x_units,
            marker_type=self._selection_marker,
    ...
    def _on_tree_context_menu(self, pos):
        ...
        if treeitem is self._treeitem_active:
            ...
            act.triggered.connect(lambda *args: self._on_discard_collected_spectra())
    ...
    def _on_save_collected_spectra(self):
        supported_formats = [self.tr("Text files (*.txt)"), self.tr("All files (*)")]
    ...
    def _on_discard_collected_spectra(self):
        ...
        confirm = QMessageBox.question(
            self,
            self.tr("Discard Collected Spectra?"),
    ...
    def _draw_spectra(self):
        ...
        if self._x_autoticks:
            ...
            if len(ticks) > TICK_THRESHOLD:
                msg = (
                    f"Configured X major tick interval {self._x_major_tick_interval} "
            ...
            if len(ticks) > TICK_THRESHOLD:
                msg = (
                    f"Configured X minor tick interval {self._x_minor_tick_interval} "
                    + f"generates {len(ticks)} ticks, which exceeds the "
                    + f"TICK_THRESHOLD of {TICK_THRESHOLD}.  Not drawing ticks."
                )
                warnings.warn(msg)
        ...
        if self._y_autoticks:
            ...
            if len(ticks) > TICK_THRESHOLD:
                msg = (
                    f"Configured Y major tick interval {self._y_major_tick_interval} "
                    + f"generates {len(ticks)} ticks, which exceeds the "
                    + f"TICK_THRESHOLD of {TICK_THRESHOLD}.  Not drawing ticks."
                )
                warnings.warn(msg)
            ...
            if len(ticks) > TICK_THRESHOLD:
                msg = (
                    f"Configured Y minor tick interval {self._y_minor_tick_interval} "
                    + f"generates {len(ticks)} ticks, which exceeds the "
                    + f"TICK_THRESHOLD of {TICK_THRESHOLD}.  Not drawing ticks."
                )
                warnings.warn(msg)
