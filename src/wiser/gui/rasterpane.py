...
from .ui_selection_multi_pixel import (
    MultiPixelSelectionCreator,
    MultiPixelSelectionEditor,
)

from wiser.raster.dataset import GeographicLinkState
...
class TiledRasterView(RasterView):
    ...
    def __init__(
        ...
        self._act_stretch_builder = add_toolbar_action(
            self._toolbar,
            ":/icons/stretch-builder.svg",
            self.tr("Stretch builder"),
            self,
        )
...
class RasterPane(QWidget):
    ...
    def _init_zoom_tools(self):
        ...
        # Zoom In
        self._act_zoom_in = add_toolbar_action(
            self._toolbar,
            ":/icons/zoom-in.svg",
            self.tr("Zoom in"),
            self,
        ...
        # Zoom Out
        self._act_zoom_out = add_toolbar_action(
            self._toolbar,
            ":/icons/zoom-out.svg",
            self.tr("Zoom out"),
            self,
    ...
    def _init_select_tools(self):
        ...
        self._act_create_roi = add_toolbar_action(
            self._toolbar,
            ":/icons/add-roi.svg",
            self.tr("Add new Region of Interest"),
            self,
            QKeySequence.ZoomIn,
        )
    ...
    def _init_rasterviews(
        self,
        num_views: Tuple[int, int] = (1, 1),
        rasterview_class: TiledRasterView = TiledRasterView,
    ):
        """
    ...
    def create_viewport_highlight_dictionary(
        self,
        viewport: Union[List[QRect], QRect, QRectF],
    ...
    def set_pixel_highlight(
        self,
        pixel_sel: Optional[SinglePixelSelection],
        ...
        for rasterview in self._rasterviews.values():
            ...
            # of reference
            target_pixel = reference_pixel_to_target_pixel_ds(
                (coord.x(), coord.y()),
    ...
    def _on_add_selection_to_roi(self, act):
        ...
        if selection_type == SelectionType.RECTANGLE:
            ...
            ok, pred_text = QInputDialog.getText(
                self,
    ...
    def _on_edit_roi_selection_geometry(self, roi, sel_index, rasterview) -> None:
        ...
        if selection_type == SelectionType.RECTANGLE:
            self._task_delegate = RectangleSelectionEditor(roi, sel, self, rasterview)

        elif selection_type == SelectionType.POLYGON:
            self._task_delegate = PolygonSelectionEditor(roi, sel, self, rasterview)

        elif selection_type == SelectionType.MULTI_PIXEL:
            self._task_delegate = MultiPixelSelectionEditor(roi, sel, self, rasterview)
    ...
    def _on_delete_roi_selection_geometry(self, roi, sel_index) -> None:
        ...
        result = QMessageBox.question(
            ...
            self.tr(
                "Are you sure you wish to delete selection {0} "
                + 'from the ROI "{1}"?  This cannot be undone.'
            ).format(sel_index, name),
        )
    ...
    def _on_export_roi_pixel_spectra(
        ...
        if len(pixels) > 200:
            result = QMessageBox.question(
                ...
                self.tr(
                    "This ROI has {0} pixels.  Are you sure you wish to "
                    + "output the spectra of all pixels?"
                ).format(len(pixels)),
            )
    ...
    def draw_highlights(
        self,
        highlights: List[Union[QRect, QRectF, QPolygon]],
        painter: QPainter,
        widget: QWidget,
    ):
        """
    ...
    def _transform_viewport_to_polygon(
        self,
        viewport: QRect,
