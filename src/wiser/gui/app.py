    ...
    def generate_project_info(self):
        ...
        *   Qt application state including window geometry and open/close state
        ...
        for data_set in self._app_state.get_datasets():
            ds_info = {"files": data_set.get_filepaths()}
    ...
    def show_bandmath_dialog(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                (result_type, result) = bandmath.eval_bandmath_expr(
                    expression,
                    expr_info,
                    result_name,
                    self._data_cache,
                ...
                if result_type == RasterDataSet:
                    ...
                    if expr_info.spatial_metadata_source:
                        new_dataset.copy_spatial_metadata(
                            expr_info.spatial_metadata_source.value
                        )

                    if expr_info.spectral_metadata_source:
                        new_dataset.copy_spectral_metadata(
                            expr_info.spectral_metadata_source.value
                        )

                    self._app_state.add_dataset(new_dataset)

                elif result_type == bandmath.VariableType.IMAGE_CUBE:
                    new_dataset = loader.dataset_from_numpy_array(
                        result, self._data_cache
                    )
    ...
    def show_reference_creator_dialog(self, in_test_mode=False):
        ...
        if not in_test_mode:
            if self._crs_creator_dialog.exec_() == QDialog.Accepted:
                print("Reference creator accepted!")
        else:
