...
class MainViewWidget(RasterPane):
    ...
    def _init_view_tools(self):
        ...
        self._act_link_view_scroll = add_toolbar_action(
            self._toolbar,
            ":/icons/link-scroll.svg",
            self.tr("Link view scrolling"),
            self,
        )
    ...
    def _init_zoom_tools(self):
        ...
        # Zoom to Actual Size
        self._act_zoom_to_actual = add_toolbar_action(
            self._toolbar,
            ":/icons/zoom-to-actual.svg",
            self.tr("Zoom to actual size"),
            self,
    ...
    def _open_mtmf_dialog(self, rasterview):
        dataset = rasterview.get_raster_data()
        dataset_id = None if dataset is None else dataset.get_id()
        dlg = MTMFDialog(self._app_state, self._app_services, parent=self)
        dlg.select_image_cube_dataset(dataset_id)
        dlg.exec_()
    ...
    def on_scatter_plot_2D(self, rasterview=None, testing=False):
        ...
        if (
            ...
            return
        self._interactive_scatter_plot_dialog = ScatterPlot2DDialog(
            self._make_interactive_scatter_plot_highlights,
    ...
    def _on_link_view_scroll(self, checked):
        ...
        (
            linkable,
            link_type,
        ) = self._app_state.multiple_displayed_datasets_link_compatible()
        ...
        if checked and not linkable:
            ...
            QMessageBox.information(
                self,
                self.tr("Cannot Link Views"),
                self.tr(
                    "Cannot link views unless multiple datasets\n"
    ...
    def _draw_viewport_highlight(self, rasterview, widget, paint_event):
        ...
        if self._viewport_highlight is not None:
            assert (
                len(list(self._viewport_highlight.values())) == 1
            ), "self._viewport_highlight has more than 1 entry"
            ...
            for reference_ds_id, viewports in self._viewport_highlight.items():
                ...
                for viewport in viewports:
                    ...
                    if link_state == GeographicLinkState.NO_LINK:
                        ...
                        transformed_viewport = self._transform_viewport_to_polygon(
                            viewport, reference_ds, target_ds
                        )
