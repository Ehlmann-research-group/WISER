...
def export_raster_image(image_data: np.ndarray,
                        filename: str,
                        output_format: str,
                        output_attrs: Dict = {},
    ...
    if scale != 1.0:
        im = im.resize(size=(int(width * scale + 0.5), int(height * scale + 0.5)),
...
class ExportImageDialog(QDialog):
    ...
    def accept(self):
        ...
        if x + width > dataset.get_width():
            QMessageBox.critical(self, self.tr('Invalid X coordinate and width'),
                self.tr('X coordinate + width must fall within the image'))
            return

        if y + height > dataset.get_height():
            QMessageBox.critical(self, self.tr('Invalid Y coordinate and height'),
        ...
        try:
            ...
            export_raster_image(image_data, filename, format, format_attrs,
                crop=(x, y, width, height), scale=scale)
